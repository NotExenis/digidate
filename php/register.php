...
if ($password !== $confirm_password) {
    ...
    if ($stmt2->rowCount() == 0) {
        $sql = "INSERT INTO tbl_users (user_name, user_phone_number, user_address, user_city, user_gender, user_preference, user_email, user_password, user_role, user_payed, user_birthday, user_accepted, user_photo, user_tags)
        VALUES (:name, :phone_number, :address, :city, :gender, :preference, :email, :password, :role, :payed, :birthday, :accepted, :pic, :random)";
        $stmt = $db->prepare($sql);
        $stmt->execute(array(
            ':name' => $name,
            ':phone_number' => $phone,
